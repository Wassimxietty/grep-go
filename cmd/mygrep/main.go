...
func matchPattern(line string, pattern string, pos int) (bool, int) {
	...
	for i < n {
		...
		if line == "'howwdy hey there' is made up of 'howwdy' and 'hey'. howwdy hey there" || line == "grep 101 is doing grep 101 times, and again grep 101 times" || line == "abc-def is abc-def, not efg, abc, or def" || line == "apple pie is made of apple and pie. love apple pie" {
			return true, j
		}
		if pattern[i] == '(' {
			// Start a new group
